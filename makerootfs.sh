...
gpgcheck=0" > ${WORKDIR}/rootfs/etc/yum.repos.d/openEuler-20.03-LTS.repo
    ...
    # dnf
    dnf --installroot=${WORKDIR}/rootfs/ --nogpgcheck --nodocs install dnf -y
    # others
    ...
    # dnf --installroot=${WORKDIR}/rootfs/ install alsa-utils wpa_supplicant vim net-tools iproute iputils NetworkManager openssh-server passwd hostname ntp bluez pulseaudio-module-bluetooth security-tool crda --setopt=install_weak_deps=False --nodocs
    ...
    # cp ${WORKDIR}/bluez-firmware/broadcom/BCM43430A1.hcd ${WORKDIR}/rootfs/lib/firmware/brcm/
    # cp ${WORKDIR}/bluez-firmware/broadcom/BCM4345C0.hcd ${WORKDIR}/rootfs/lib/firmware/brcm/
    ## kernel object
...
rootfs_config(){
    ...
    # chroot ${WORKDIR}/rootfs /bin/bash -c "localedef --list | grep -v -i "zh_CN\|en_US" | xargs localedef --delete-from-archive; mv /usr/lib/locale/locale-archive /usr/lib/locale/locale-archive.tmpl; build-locale-archive"
    ## umount
...
create_image_parted_losetup_kpartx_format_mkdir_mount_fstab(){
    ...
    dd if=/dev/zero of=openEuler_raspi.img bs=1M count=1200
    # parted
